...
let texts = [
  [
    ...
    "🖐 oie.",
    "Pode desenhar aqui!",
    '<span class="icon pintaricon"></span> 👆️',
  ],
...
function startup() {
  ...
  window.onkeydown = function (event) {
    ...
    if (
      ...
      activeEl.tagName != "EMOJI-PICKER" &&
      activeEl.type != "text"
    ) {
      if (mode != "emoji" && keyCtrl == false && keyAlt == false) {
        if (
          mode == "texto" &&
          !iD("dropdown-options").classList.contains("esconde")
        ) {
          return;
        }
  ...
  iD("globalComposite").innerHTML = `<span class="icon2 paintovericon"></span>`;
  ...
  if (event.dataTransfer.files.length > 0 && dataTransfer[0] == "c") {
    ...
    for (let i = 0; i < event.dataTransfer.files.length; i++) {
      ...
      img.onload = function () {
        ...
        modeTo("selecionar");
        updateClipboard();
        setTimeout(() => changeImage2(clipboard.length - 1), 10);
      };
    }
  }
}
...
function transformClip(x = canvas.width / 2, y = canvas.height / 2) {
  ...
  setTimeout(() => {
    ...
    ctxF.drawImage(
      canvasRender,
      x - canvasRender.width / 2,
      y - canvasRender.height / 2
    );
  }, 80);
}
