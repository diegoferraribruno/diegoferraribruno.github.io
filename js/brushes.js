...
function toggleDinamicBrush() {
  ...
  if (dinamicBrush == true) {
    iD("dinamicbrush").innerHTML =
      '<span class="icon dinamicbrushicon"><span class="minicheck"></span></span>';
    Alert(
      '<span title="dinamicbrush" class="dinamicbrush"> </span> ' +
        alerts[language][25] +
        "<br>" +
        alerts[language][7]
    );
    ...
    iD("dinamicbrush").innerHTML =
      '<span class="icon dinamicbrushicon"></span>';
    Alert(
      '<span title="dinamicbrush" class="dinamicbrush"> </span> ' +
        alerts[language][25] +
        "<br>" +
...
function criaCustom(fromclipboard = false) {
  if (fromclipboard && clipboard.length == 0) {
    Alert(
      '<span class="icon clipboardicon"></span> ' + textos[language]["103"],
      4
    );
    return;
  }
...
function strokeSizeRange(value) {
  setStrokeSize(stroke_range[value]);
}
...
function addFavBrush() {
  brushName =
    "" + lastbrush + "-" + strokeWidth + "-" + strokeHeight + "-" + strokeColor;
  setTimeout(() => {
    ...
    if (!existe && mode != "picker") {
      ...
      if (changedBrush == false) {
        ...
        setTimeout(() => {
          ...
          favBrushButton.setAttribute(
            "onmousedown",
            "favBrush('" + brushName + "')"
          );
          favBrushButton.appendChild(favbrush);
          if (strokeScale > 10) {
            favBrushButton.innerHTML +=
              "<span class='favbrush'>" + strokeWidth + "</span>";
          } else {
            favBrushButton.innerHTML +=
              "<span style='display:block; position:relative; margin-top:-40px; margin-right:auto; margin-left:auto; text-aling:center; color: #000000cc; font-size:0.75em;'>" +
              strokeWidth +
              "</span>";
          }
...
function drawBrush2(GCO, x1, y1, x2, y2, strokeScale, cont = ctxF) {
  ...
  if (pixelGood) {
    start = {
      x: (x1 / zoomFactor) * zoomFactor + 1,
      y: (y1 / zoomFactor) * zoomFactor + 1,
    };
    end = {
      x: (x2 / zoomFactor) * zoomFactor + 1,
      y: (y2 / zoomFactor) * zoomFactor + 1,
    };
  } else {
    start = { x: x1, y: y1 };
    end = { x: x2, y: y2 };
...
function isRainbowAB() {
  if (rainbowAB) {
    ...
    if (hueinvert) {
      if (
        colorAbigger &&
        value > rainbowABcolors[1].hue &&
        value < rainbowABcolors[0].hue
      ) {
    ...
    if (satAbigger) {
      if (
        value > +rainbowABcolors[0].saturation ||
        value < +rainbowABcolors[1].hue
      ) {
    ...
    if (+rainbowABcolors[0].lightness != +rainbowABcolors[1].lightness) {
      ...
      if (lightAbigger) {
        if (
          value > +rainbowABcolors[0].lightness ||
          value < +rainbowABcolors[1].lightness
        ) {
    ...
    if (+rainbowABcolors[0].alpha != +rainbowABcolors[1].alpha) {
      ...
      if (alphaAbigger) {
        if (
          value > +rainbowABcolors[0].alpha ||
          value < +rainbowABcolors[1].alpha
        ) {
